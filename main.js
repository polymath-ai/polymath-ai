...
//  Time for some help
// --------------------------------------------------------------------------
const DEFAULT_TOKEN_COUNT = 4000;
...
class Polymath {
    ...
    constructor(options) {
        ...
        this.promptTemplate = options.promptTemplate || "Answer the question as truthfully as possible using the provided context, and if the answer is not contained within the text below, say \"I don't know\"\n\nnContext:{context}\n\nestion: {query}\n\nAnswer:";
    }
    ...
    async completion(query, polymathResults) {
        if (!polymathResults) { // get the polymath results here
            polymathResults = await this.results(query);
        }

        let prompt = this.promptTemplate.replace("{context}", polymathResults.context).replace("{query}", query);
        ...
        try {
            ...
            return {
                ...
                infos: removeDuplicateInfo(polymathResults.bits.map(entry => entry.info)),
